...
- 当前远端模型注册表已经切到父子表最终态，但首批模型数据仍偏少
...
- 完成模型注册表首轮 seed 补齐：
  - 补充 `OpenAI compatible` 的首批常用模型 seed 数据
  - 继续补充 `Gemini` 的常用模型 seed 数据
  - 校对各模型能力标记与实际 provider 行为
  - 明确多 provider 下的默认模型策略
- 继续推进 `Phase 4` 的基础扩展位：
...
- 模型注册表最终态已在远端 Supabase 落库，但当前仅有一个 Gemini seed
- 前端模型选择仍处于第一版，尚未完成持久化策略
- 深色模式还未正式进入实现
...
- 当前项目已经完成数据库驱动聊天主链路，且模型注册表最终态已在远端 Supabase 落地；接下来应以“补齐首批模型 seed + 收口模型选择体验 + 推进生成能力升级”为主线，同时补齐 `Phase 3.6` 的验收整理。
