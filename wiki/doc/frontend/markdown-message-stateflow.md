...
- 原始正文进入 ReactMarkdown 前的单美元误匹配兜底
...
在送入 `ReactMarkdown` 前，组件会先做一层很窄的单美元误匹配兜底：

- 如果 `$...$` 中间混入中文自然语言或中文标点
- 说明这更可能是普通文本里的美元符号被误配对
- 组件会转义这一对 `$`，避免 KaTeX 把整段中文当公式

正常 `$x$`、`$99^2$`、`$4 \times 99$`、`$$...$$` 不应被这层逻辑改写。
...
`MarkdownMessage` 的状态流本质上是一个分发过程：先对中文段落里的单美元误匹配做窄范围兜底，再把 `content` 解析后，根据节点类型把正文、表格、公式、行内代码和块级代码送进不同的渲染通道。
