...
`Phase 4.4` 把文件与图片输入组织到消息级 metadata 中。

这里可以理解为附件能力的数据库与 Storage 契约。附件上传、编辑、重新生成、分支、历史恢复和未引用对象清理仍需要继续用真实数据回归。
...
清理失败不会阻断消息保存；后续如果真实使用中出现残留，可再补后台清理任务。

编辑带附件 user 消息时，代码路径优先调用数据库 RPC 完成“更新目标消息 + 删除后续消息”。如果 RPC 调用失败，会 fallback 到普通 update + delete 路径，避免编辑链路被 RPC 单点阻断。后续仍建议继续复测 RPC 与 fallback 的一致性。

附件下载阶段如果遇到 Supabase 偶发网络错误，会进行短重试。重试后仍失败时，聊天链路会把底层 `fetch failed` 翻译为“Supabase 连接暂时不稳定”一类用户可读错误，避免把 Node / TLS 细节直接写入 assistant 消息。
...
- `googleSearch` 与 `urlContext` 是否启用，只取决于会话联网开关和这次 URL Context 输入
- 附件存在本身不会禁用联网工具
